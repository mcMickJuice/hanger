...
function partition<T>(collection: T[], condition: (item: T) => boolean): [T[], T[]] {
  const unFound: T[] = []
  const found: T[] = []
...
export async function fetchMealSuggestions(limit: number, excludeMealIds: string[]): Promise<string[]> {
